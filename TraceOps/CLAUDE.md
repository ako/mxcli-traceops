...
```bash
./mxcli -p TraceOps.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p TraceOps.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check TraceOps.mpr   # Validate project
./mxcli docker check -p TraceOps.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p TraceOps.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p TraceOps.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p TraceOps.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p TraceOps.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p TraceOps.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'TraceOps.mpr';
```
...
./mxcli check script.mdl -p TraceOps.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p TraceOps.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p TraceOps.mpr

# With colored output
./mxcli lint -p TraceOps.mpr --color

# List available rules
./mxcli lint -p TraceOps.mpr --list-rules

# Output as SARIF
./mxcli lint -p TraceOps.mpr --format sarif > results.sarif
```
...
./mxcli report -p TraceOps.mpr

# JSON report
./mxcli report -p TraceOps.mpr --format json

# HTML report
./mxcli report -p TraceOps.mpr --format html
```
